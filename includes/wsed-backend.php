...
class Woo_Set_Event_Date_Backend{
	...
	public function __construct(){
		...
		// Save Fields
		add_action('woocommerce_process_product_meta', array($this, 'wsed_dates_save'),20,1);

		// Include css and javascript
		add_action('admin_footer', array( $this, 'wsed_product_style_script'));
	}
	...
	public function wsed_dates_fields(){
		...
		$i=1;
		while(!empty(get_post_meta( $post->ID, 'wsed_event_date_'.$i, true )) && get_post_meta( $post->ID, 'wsed_event_date_'.$i, true ) != ""){	
		...
		echo "<div class='wsed_add_additional_date' onclick='add_additional_date()'>" . __('Add new date','woo-set-event-date') . "</div>";
		echo "<script>
					var  i = $i;
					...
					var date_placeholder = '". __('Format: YYYY/MM/DD', 'woo-set-event-date')."';
					...
					function add_additional_date(){
						...
						var new_option_date = document.createElement('input');
							...
							new_option_date.setAttribute('placeholder', date_placeholder);
	...
	public function wsed_dates_save($post_id){	
		$isDate = 0;
		$i=1;
		while(isset($_POST['wsed_event_date_'.$i])){
			...
			update_post_meta($post_id, 'wsed_event_spots_'.$i, !empty($_POST['wsed_event_spots_'.$i]) ? $_POST['wsed_event_spots_'.$i] : 0);	
	...
	public function wsed_product_style_script(){
		echo '<style>';
		echo '#woocommerce-product-data ul.wc-tabs li a::before{content: "\f508";}';
		echo '.wsed_add_additional_date{width: fit-content;padding: 10px 20px;background-color: #c9c9c9;color: #000;border-radius: 20px;margin-left: 20px;margin-bottom: 20px;cursor:pointer;}';
		echo '</style>';
	}
}
